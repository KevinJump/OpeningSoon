...
namespace Jumoo.OpeningSoon.Installer
{
    ...
    /// 
    /// and this one is direct copy of Warren Buckley's version
    /// https://github.com/warrenbuckley/Analytics/blob/develop/Analytics/TranslationHelper.cs
    /// </summary>
    public class TranslationHelper 
    {
        // Set the path of the language files directory
        private const string UmbracoLangPath = "~/umbraco/config/lang/";
        private const string AnalyticsLangPath = "~/App_Plugins/OpeningSoon/Lang/";
        ...
        public static void AddTranslations()
        {
            ...
            LogHelper.Info<TranslationHelper>(string.Format("{0} Opening soon Plugin language files to be merged into Umbraco language files", analyticsFiles.Count()));
            ...
            LogHelper.Info<TranslationHelper>(string.Format("{0} Umbraco language files that match up with the opening soon language files", existingLangs.Count()));
        ...
        {
            var analyticsFiles = GetAnalyticsLanguageFiles();
            LogHelper.Info<TranslationHelper>(string.Format("{0} Opening Soon Plugin language files to be removed into Umbraco language files", analyticsFiles.Count()));
            ...
            //Check which language filenames that we have match up
            var existingLangs = GetUmbracoLanguageFilesToInsertLocalizationData();
            LogHelper.Info<TranslationHelper>(string.Format("{0} Umbraco language files that match up with opening soon language files",existingLangs.Count()));
